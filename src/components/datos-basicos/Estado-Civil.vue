    ...
    <v-container>
      <v-card>
        ...
        >
          <template v-slot:top>
            <v-toolbar flat>
              <v-toolbar-title>Registros Estado Civil</v-toolbar-title>
                  ...
                  <v-card-text>
                    <v-container>
                      <v-row>
                        ...
                        <v-col cols="12" sm="6" md="4">
                          <v-text-field
                            v-model="estadoscivilesItem.valorRegistro"
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      this.estadoscivilesItem = Object.assign({}, item);
      this.dialog = true;
    },
    ...
    deleteItemConfirm() {
      ...
      console.log(this.estadoscivilesItem.id);
      axios
        .delete(url + "api/estados-civiles/" + this.estadoscivilesItem.id)
        .then((response) => {
    ...
    close() {
      this.dialog = false;
      this.$nextTick(() => {
        this.estadoscivilesItem = Object.assign({}, this.defaultItem);
        this.editedIndex = -1;
      });
    },
    ...
    save() {
      ...
      if (this.editedIndex > -1) {
        axios
          .put(
            url + "api/estados-civiles/" + this.estadoscivilesItem.id,
            this.estadoscivilesItem
          )
          .then((response) => {
            if (response.status == 200) {
        ...
        console.log(url + "api/estados-civiles/" + this.estadoscivilesItem.id);
        console.log(this.estadoscivilesItem.valorRegistro);
        axios
          .post(url + "api/estados-civiles", {
            valorRegistro: this.estadoscivilesItem.valorRegistro,
          })
          .then((response) => {
